...
export function DonationTimeline({ donorId }: { donorId: number }) {
  const { data: history, isLoading, isError } = useDonorHistory(donorId);
  ...
  if (isError) {
    return <div className="text-muted-foreground pt-4">Failed to load donation history.</div>;
  }

  if (!history || history.length === 0) {
    return (
      <div className="text-center p-8 bg-muted/20 rounded-lg border border-dashed mt-4">
        <p className="text-muted-foreground">No donation history recorded yet.</p>
      </div>
    );
  }
          ...
          <div className="flex-1 pb-6">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-semibold text-sm">
                {format(new Date(record.donationDate), "MMMM d, yyyy")}
              </span>
            </div>
