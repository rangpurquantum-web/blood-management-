...
export default function SuperAdminLoginPage() {
  ...
  async function onSubmit(data: LoginFormValues) {
    ...
    try {
      ...
      // Confirm this account is actually a SuperAdmin before letting them in here.
      const session = await getSession();

      if (!session?.user?.isSuperAdmin) {
        toast.error("This login is for SuperAdmin accounts only.");
        // Sign the non-superadmin session back out so they're not left half-authenticated here.
    ...
    <main className="relative flex min-h-screen items-center justify-center p-4 sm:p-8">
      ...
      <Card className="w-full max-w-md border-muted/40 bg-card/60 shadow-2xl backdrop-blur-xl">
        <CardHeader className="space-y-2 text-center pb-8">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-slate-700 to-slate-900 text-white shadow-lg">
            <ShieldCheck className="h-8 w-8" />
          </div>
          <CardTitle className="text-3xl font-bold tracking-tight">SuperAdmin</CardTitle>
        ...
        <CardFooter className="flex flex-col items-center gap-3 border-t border-border/40 p-4">
          <p className="text-xs text-muted-foreground">
            Restricted access. All logins are audited.
          </p>
        </CardFooter>
      </Card>
    </main>
